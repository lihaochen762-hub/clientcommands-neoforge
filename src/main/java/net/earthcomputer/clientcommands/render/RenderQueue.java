...
public class RenderQueue {
    ...
    static {
        ...
        LevelRenderEvents.END_EXTRACTION.register(context -> {
            ...
            // Fabric API's interface injection is not applied to the compile classpath by NeoGradle,
            // so the injected methods are reached through an explicit cast.
            ((FabricRenderState) (Object) context.levelState()).setData(RENDER_STATES_KEY, renderStates);
        });
    ...
    private static void render(Layer layer, RenderType renderType, LevelRenderContext context) {
        EnumMap<Layer, List<Shape.RenderState>> renderStates = ((FabricRenderState) (Object) context.levelState()).getData(RENDER_STATES_KEY);
