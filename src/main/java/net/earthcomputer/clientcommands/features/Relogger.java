...
public class Relogger {
    ...
    static {
        ...
        // Fabric API's per-screen after-render and after-tick events are not available on
        // NeoForge, so the relog retry button is driven from NeoForge's own screen and tick events.
        NeoForge.EVENT_BUS.addListener(ScreenEvent.Render.Post.class, event -> {
            if (event.getScreen() == watchedScreen) {
                onDisconnectScreenRender(watchedScreen, event.getGuiGraphics(), event.getMouseX(), event.getMouseY(), event.getPartialTick());
            }
        });
        NeoForge.EVENT_BUS.addListener(ClientTickEvent.Post.class, event -> {
            if (watchedScreen != null && Minecraft.getInstance().gui.screen() == watchedScreen) {
                onDisconnectScreenTick(watchedScreen);
            }
        });
    }

    /**
     * Starts showing the retry progress on the given disconnect screen.
     */
    public static void watchDisconnectScreen(Screen screen) {
