...
# Embedded library: ClientArguments
...
# Embedded library: SimpleWaypoints
...
accessible method net/minecraft/client/Camera calculateFov (F)F
...
# Taken from Fabric API's transitive access wideners for 26.2. Fabric API makes these
# accessible to every mod, but the Forgified Fabric API port is built against 26.1.2 and does
# not carry them, so they are declared here to keep the source compiling unchanged.
...
accessible method net/minecraft/network/chat/contents/TranslatableContents getArgument (I)Lnet/minecraft/network/chat/FormattedText;
